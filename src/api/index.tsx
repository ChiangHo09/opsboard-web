 ...
 * @description 此文件是应用的模拟 API 层。它通过将每个数据实体（如 Tickets, Servers）
 *   封装在各自的对象中，提供了一个结构化、模块化的方式来定义和导出所有与后端
 *   交互的数据获取函数。这种模式使得 API 层更易于管理、测试和未来的扩展。
 * @modification
 *   - [性能优化]：将模拟数据数组（`ticketRows`, `serverRows`, `changelogRows`）定义为模块级别的常量，并在模块加载时只生成一次。
 *   - [性能优化]：修改 `fetchAll` 函数，使其直接返回这些预先生成好的、稳定的数组引用。此举确保了 `useQuery` 钩子能够接收到稳定的数据引用，从而允许 `React.memo` 在表格行组件上有效工作，避免不必要的表格重新渲染，解决页面切换时的卡顿问题。
...
// 【核心修改】将 ticketRows 定义为模块级别的常量，只生成一次
...
// 【核心修改】将 serverRows 定义为模块级别的常量，只生成一次
...
// 【核心修改】将 changelogRows 定义为模块级别的常量，只生成一次
