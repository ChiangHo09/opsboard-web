 ...
 *
 * 本次修改内容:
 * - 【Props 命名修复】修正了在桌面端弹窗中调用 `TemplateModalContent` 组件时传递的 prop 名称。
 * - **问题根源**:
 *   在 `TemplateModalContent` 组件将其期望的 prop 从 `id` 修改为 `itemId` 后，此处的调用未同步更新，导致了类型不匹配的错误。
 * - **解决方案**:
 *   将 `<TemplateModalContent id={itemId} />` 修改为 `<TemplateModalContent itemId={itemId} />`。
 * - **最终效果**:
 *   桌面端弹窗的调用现在与组件的 props 定义完全匹配，所有类型错误都已消除。
 */
            ...
            setModalConfig({
                content: (
                    <Suspense fallback={<Box sx={{
                        ...
                        {/* 【核心修复】将传递的 prop 名称从 id 修改为 itemId */}
