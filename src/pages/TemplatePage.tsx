 ...
 *
 * 本次修改内容:
 * - 【类型安全修复】为 `DataTable` 组件的 props (onPageChange, onRowsPerPageChange) 的回调参数添加了显式类型定义。
 * - **解决方案**:
 *   - `onPageChange`: 为 `newPage` 参数添加 `number` 类型。
 *   - `onRowsPerPageChange`: 为 `event` 参数添加 `React.ChangeEvent<HTMLInputElement>` 类型。
 *   - 这些改动解决了 TypeScript 报出的 `TS7006: Parameter '...' implicitly has an 'any' type.` 错误。
 */
